...
const EXECUTE_SKILL_BODY = [
  ...
  '- Call `tools.executor.files.importLocal({ path: "/absolute/path/file.pdf" })` to read a regular file up to 5 MiB from the daemon\'s disk and receive `{ ok: true, data: ToolFile }` (base64 bytes in `data.data`) or `{ ok: false, error }`.',
