...
const EXECUTE_SKILL_BODY = [
  ...
  "## Workflow",
  "",
  '1. If the request names an account, environment, or role, first use `tools.executor.coreTools.connections.list({ integration: "<integration>" })` and select its exact `address`; ask the user if the choice is ambiguous.',
  '2. `const { items: matches } = await tools.search({ query: "<intent + key nouns>", connection: selectedAddress, limit: 12 });` Omit `connection` only when the request is not connection-specific.',
  ...
  "- Pass that exact `address` as `tools.search({ connection: address, query })` to search only one connection. Never fall back to another connection when a scoped search or call fails.",
