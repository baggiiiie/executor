...
const EXECUTE_SKILL_BODY = [
  ...
  '- Call `tools.executor.files.importLocal({ path: "/absolute/path/file.pdf" })` to read a regular file up to 5 MiB from local disk, `tools.executor.files.exportLocal({ file: toolFile, path: "/absolute/path/file.pdf" })` to save a `ToolFile`; both return `{ ok: true, data: ToolFile }` (base64 bytes in `data.data`) or `{ ok: false, error }`.',
...
const CREATE_ARTIFACT_SKILL_BODY = [
  ...
  "- Use discovered result shapes exactly. Tool calls return `{ ok: true, data: payload }` or `{ ok: false, error }`; the artifact proxy preserves this tool-result envelope. For `const query = useQuery(tools.<integration>.<tool>.queryOptions(args))`, `query.data` is the envelope and `query.data.data` is the successful payload. If the payload is `{ issues: [...] }`, read `query.data.data.issues` after checking `query.data?.ok === true`, not `query.data.issues`.",
  "- Expected tool failures resolve as `{ ok: false, error }`, so `query.error` alone is not enough. Render `ArtifactError` with `query.data.error` when `query.data?.ok === false`, before considering an empty state; also handle loading and `query.error`. Never turn a failed tool result into `[]` or a success-looking empty state. Mutation results and infinite-query pages preserve the same envelope: check `result.ok` / `lastPage.ok` before reading `.data`.",
  ...
  "- Always render the artifact's root, title, filters, metric labels, and data containers immediately; never replace the whole artifact with an early `return <ArtifactLoading />`. Conditionalize only each data region between `ArtifactLoading`, initial `ArtifactError`, `ArtifactEmpty`, and populated content, so surrounding UI and independent panels stay mounted.",
  "- Keep existing data visible during refetches: gate the full-region `ArtifactError` on `query.error && query.data === undefined`. When a refetch fails with data present, keep the rows and surface `query.error.message` inline (a small `Alert` above them) instead of hiding the error or replacing the data.",
  "- Titles paint before data, so do not put data-derived counts in them (`Domains ({rows.length})` reads as `Domains (0)` while loading); render counts inside the data region, next to the rows.",
  "",
  "## Paginated Reads",
  "",
  ...
  "and returns `{ ok: true, data: { domains, pagination: { next } } }` on success):",
  "",
  "```jsx",
  "function App() {",
  ...
  "  );",
  "",
  "  const pages = domains.data?.pages ?? [];",
  "  const failedPage = pages.find((page) => page.ok === false);",
  "  const rows = pages.flatMap((page) => page.ok ? page.data.domains : []);",
  "",
  "  return (",
  "    <Card>",
  "      <CardHeader><CardTitle>Domains</CardTitle></CardHeader>",
  "      <CardContent>",
  '        {domains.isLoading ? <ArtifactLoading variant="table" rows={5} /> :',
  "         domains.error && domains.data === undefined ? <ArtifactError error={domains.error} onRetry={domains.refetch} /> :",
...
const ARTIFACT_STYLE_SKILL_BODY = [
  ...
  "```",
  "",
  "The three data-region states are already in scope — keep the surrounding title and controls mounted, and use these where the data belongs:",
  "",
  "```jsx",
  "<Card>",
  "  <CardHeader><CardTitle>Deployments</CardTitle></CardHeader>",
  "  <CardContent>",
  '    {query.isLoading ? <ArtifactLoading variant="table" rows={5} /> :',
  "     query.error && query.data === undefined ? <ArtifactError error={query.error} onRetry={query.refetch} /> :",
  ...
  "     <Table><TableBody>",
  "       {rows.map((row) => <TableRow key={row.id}><TableCell>{row.name}</TableCell></TableRow>)}",
  "     </TableBody></Table>}",
  "  </CardContent>",
  "</Card>",
