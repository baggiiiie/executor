...
const CREATE_ARTIFACT_SKILL_BODY = [
  ...
  "- Use discovered result shapes exactly. Successful tool calls preserve the `{ ok: true, data: payload }` envelope. For `const query = useQuery(tools.<integration>.<tool>.queryOptions(args))`, `query.data` is that envelope and `query.data.data` is the payload. If the payload is `{ issues: [...] }`, read `query.data?.ok === true ? query.data.data.issues : []`, not `query.data.issues`.",
  "- Failed tool calls reject instead of entering `query.data`, so TanStack exposes them as `query.error` / `mutation.error` and invokes mutation `onError`. Render `ArtifactError` for an initial query failure before considering an empty state. Never turn `query.error` into `[]` or a success-looking empty state. Infinite-query pages contain only successful envelopes; a failed next-page or refetch request is exposed as `query.error` while existing pages remain in `query.data`.",
  ...
  '            {rows.length === 0 ? <ArtifactEmpty title="No domains yet" /> : (',
  "              <>",
  "                <Table>",
  "                  <TableBody>",
  "                    {rows.map((domain) => (",
